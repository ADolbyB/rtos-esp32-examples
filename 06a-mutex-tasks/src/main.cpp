 ...
 * In this program, a parameter is passed to a task using a mutex.
 * Note that this is meant to be a hack that waits for input from the user in the
 * setup() function before 'delay_arg' goes out of scope when setup() ends.
 */
...
void setup()                                                // setup() & loop() run on Core 1 w/ Priority 1
{
    ...
    Serial.println("\n\n\t=>> FreeRTOS Mutex & Task Paramter Hack <<=");
    ...
    xSemaphoreTake(mutex, portMAX_DELAY);                   // Take back mutex
...
{
    xSemaphoreTake(mutex, portMAX_DELAY);                   // Take mutex to access critical resource
